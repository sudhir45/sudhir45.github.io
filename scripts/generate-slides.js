...
const PUBLIC_DIR = path.join(PROJECT_DIR, 'public', 'presentations');
const TEMP_DIR = path.join(PROJECT_DIR, '.temp-slides');

function resolveProjectSourcePath(sourcePath) {
	const resolved = path.resolve(PROJECT_DIR, sourcePath);
	const relative = path.relative(PROJECT_DIR, resolved);

	if (path.isAbsolute(relative) || relative === '..' || relative.startsWith(`..${path.sep}`)) {
		throw new Error('Presentation sourcePath must stay inside the project root');
	}
...
async function convertPptxToImages(presentation) {
	...
	let sourceFile;
	try {
		sourceFile = resolveProjectSourcePath(sourcePath);
	} catch (error) {
		console.error(`✗ Invalid source path for "${slug}": ${error.message}`);
		return 0;
	}
	...
	try {
		...
		execSync(`soffice --headless --convert-to pdf --outdir "${tempPdfDir}" "${sourceFile}"`, {
			stdio: 'pipe'
		});
...
async function updatePresentationsData(slideCountMap) {
	const dataPath = path.join(PROJECT_DIR, 'src', 'data', 'presentations.json');
